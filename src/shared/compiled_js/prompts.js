...
export class PopoutMenuPrompt {
    constructor(OnConfirm, onCancel, rightSidePreferred = true, minHorizontalGapFromClick = 20) {
        // Store callbacks
        this.onConfirm = OnConfirm;
        this.onCancel = onCancel;
    ...
    initializeBaseDOM() {
        ...
        document.addEventListener('keydown', this.escapeKeyListener);
        this.clickOffListener = (e) => {
            if (this.promptContentContainer && !this.promptContentContainer.contains(e.target)) {
                this.onCancel();
                this.closePrompt();
            }
        };
    ...
    positionWithDimensions(clickEvent, contentRect) {
...
export class ConfirmationPrompt extends FullscreenPrompt {
    constructor(promptTitle, promptHTML, confirmButtonText, cancelButtonText, onConfirm, onCancel = () => { }) {
...
export class ColorPickerPrompt extends PopoutMenuPrompt {
    constructor(initialColor, clickEvent, onConfirm, onChange, onCancel, rightSidePreferred = true, minHorizontalGapFromClick = 20) {
        // Call base class constructor
    ...
    updateColorFromCanvas(x, y, updateHex = false) {
 ...
 */
export class InputPrompt extends FullscreenPrompt {
    constructor(promptTitle, promptHTML, confirmButtonText, cancelButtonText, onConfirm, onCancel = () => { }, promptWidth, promptHeight) {
        // Call base class constructor
        super(promptTitle, (...args) => {
            onConfirm(...args);
            this.closePrompt();
        }, () => {
            onCancel();
...
export class SingleTextInputPrompt extends InputPrompt {
    constructor(promptTitle, labelHTML, defaultValue = '', confirmButtonText = 'Confirm', cancelButtonText = 'Cancel', onConfirm, onCancel = () => { }, promptWidth = 440) {
        super(promptTitle, labelHTML, confirmButtonText, cancelButtonText, (v) => onConfirm(v), onCancel, promptWidth);
...
export var FileSortType;
(function (FileSortType) {
    FileSortType[FileSortType["ALPHA"] = 0] = "ALPHA";
    FileSortType[FileSortType["DATE"] = 1] = "DATE";
    FileSortType[FileSortType["NUMBER"] = 2] = "NUMBER";
    FileSortType[FileSortType["NONE"] = 3] = "NONE";
})(FileSortType || (FileSortType = {}));
export class FileListViewerPrompt extends InputPrompt {
    get selectedFileMetadata() { return this._selectedFileMetadata; }
    set selectedFileMetadata(v) {
        this._selectedFileMetadata = v;
        if (this.deleteBtn)
            this.deleteBtn.disabled = v === null;
    }
    confirm() {
        if (this.selectedFileMetadata)
            super.confirm(this.selectedFileMetadata);
    }
    constructor(promptTitle, confirmButtonText, cancelButtonText, onConfirm, onCancel = () => { }, fileMetadatasGetPath, columnLabels = {
        'Name': FileSortType.ALPHA,
        'Date Modified': FileSortType.DATE,
        'File Size': FileSortType.NUMBER
    }, promptWidth = 820, promptHeight = 580, showUploadButton = false, uploadPath = null, uploadAcceptTypes = '', showDeleteButton = false, deleteEndpoint = null) {
        // Call base class constructor
        ...
        this.sortArrows = new Map();
        this._selectedFileMetadata = null;
    ...
    populateFileListInDOM() {
        ...
        if (this.emptyStateElement) {
            ...
            if (count === 0)
                this.emptyStateElement.textContent = 'No files found.';
        }
    }
    updateFileCount(visible, total) {
    ...
    initializeAdditionalDOM() {
        ...
        if (this.showDeleteButton && this.deleteEndpoint) {
            ...
            deleteBtn.addEventListener('click', () => {
                ...
                if (!metadata)
                    return;
                new ConfirmationPrompt('Delete File', `Are you sure you want to permanently delete <strong>${metadata.name}</strong>? This cannot be undone.`, 'Delete', 'Cancel', async () => {
                    var _a;
                    const prevText = (_a = deleteBtn.textContent) !== null && _a !== void 0 ? _a : '🗑 Delete';
                    deleteBtn.disabled = true;
        ...
        if (this.showUploadButton && this.uploadPath) {
            ...
            uploadBtn.addEventListener('click', () => {
                ...
                fileInput.type = 'file';
                if (uploadAcceptTypes)
                    fileInput.accept = uploadAcceptTypes;
                ...
                fileInput.addEventListener('change', () => {
                    ...
                    new SingleTextInputPrompt('Name Your File', 'Enter a display name for the uploaded file:', defaultName, 'Upload', 'Cancel', async (name) => {
                        var _a;
                        const trimmed = name.trim();
                        if (!trimmed)
                            return;
                        const prevText = (_a = uploadBtn.textContent) !== null && _a !== void 0 ? _a : '⬆ Upload';
        ...
        scrollArea.style.scrollbarColor = '#3a3a3a #1a1a1a';
        // Table
        ...
        searchInput.addEventListener('input', () => {
            ...
            for (const row of Array.from(this.tBody.rows)) {
                const match = ((_a = row.textContent) !== null && _a !== void 0 ? _a : '').toLowerCase().includes(term);
...
export class GeoeditFileListViewerPrompt extends FileListViewerPrompt {
    constructor(onConfirm, onCancel = () => { }) {
        // Call base class constructor
        super('Load Geoedit File', 'Load', 'Cancel', onConfirm, onCancel, "/geofence/list_metadatas", {
            'Name': FileSortType.ALPHA,
            'Date Modified': FileSortType.DATE,
            'Size': FileSortType.NUMBER,
            'UUID': FileSortType.NONE,
        }, 820, 580, true, '/geofence/upload', '.geoedit', true, '/geofence/delete');
        this.initializeAdditionalDOM();
    }
    initializeFileItemDOM(...metadata) {
        ...
        tr.onmouseenter = () => { if (tr !== this.selectedRow)
            tr.style.background = '#2c2c2c'; };
        tr.onmouseleave = () => { if (tr !== this.selectedRow)
            tr.style.background = ''; };
        tr.onclick = () => {
            if (this.selectedRow) {
                this.selectedRow.style.background = '';
                this.selectedRow.style.color = '#ddd';
                this.selectedRow.querySelectorAll('td').forEach(td => td.style.color = '');
            }
            this.selectedRow = tr;
...
export class StatusCollectionFileListViewerPrompt extends FileListViewerPrompt {
    constructor(onConfirm, onCancel = () => { }) {
        super('Status Collections', 'Load', 'Cancel', onConfirm, onCancel, '/status_collection/list_metadatas', {
            'Name': FileSortType.ALPHA,
            'Description': FileSortType.NONE,
            'Date Modified': FileSortType.DATE,
            'Size': FileSortType.NUMBER,
            'UUID': FileSortType.NONE,
        }, 940, 580, true, '/status_collection/upload', '.scollection', true, '/status_collection/delete');
        this.initializeAdditionalDOM();
    }
    initializeFileItemDOM(...metadata) {
        var _a;
        const [name, lastModified, fileSize, UUID, fullMetadata] = metadata;
        const description = (_a = fullMetadata.description) !== null && _a !== void 0 ? _a : '';
        const tr = document.createElement('tr');
        tr.style.cssText = `cursor: pointer; color: #ddd; border-bottom: 1px solid #2a2a2a;`;
        tr.tabIndex = 0;
        const cellStyle = `padding: 9px 14px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;`;
    ...
    getSortKey(metadata, columnLabel) {
        ...
        if (label.includes('description'))
            return ((_a = metadata.description) !== null && _a !== void 0 ? _a : '').toLowerCase();
        return super.getSortKey(metadata, columnLabel);
    }
}
export class InterfaceCollectionFileListViewerPrompt extends FileListViewerPrompt {
    constructor(onConfirm, onCancel = () => { }) {
        super('Interface Collections', 'Load', 'Cancel', onConfirm, onCancel, '/interface_collection/list_metadatas', {
            'Name': FileSortType.ALPHA,
            'Date Modified': FileSortType.DATE,
            'Size': FileSortType.NUMBER,
            'UUID': FileSortType.NONE,
...
export class MediaFileListViewerPrompt extends FileListViewerPrompt {
    ...
    constructor(mediaType, onConfirm, onCancel = () => { }) {
        const title = mediaType === 'image' ? 'Select Image' : 'Select Audio File';
        super(title, 'Select', 'Cancel', onConfirm, onCancel, `/media/${mediaType}/list_metadatas`, {
            'Preview': FileSortType.NONE,
            'Name': FileSortType.ALPHA,
            'Date Modified': FileSortType.DATE,
            'Size': FileSortType.NUMBER,
        }, 860, 580, true, `/media/${mediaType}/upload`, mediaType === 'image' ? 'image/*' : 'audio/*', true, `/media/${mediaType}/delete`);
        this.activeAudio = null;
        this.activePlayBtn = null;
        this.mediaType = mediaType;
        this.initializeAdditionalDOM();
    }
    initializeFileItemDOM(...args) {
        const [name, lastModified, fileSize, _uuid, fullMetadata] = args;
        const tr = document.createElement('tr');
        tr.style.cssText = `cursor: pointer; color: #ddd; border-bottom: 1px solid #2a2a2a;`;
        tr.tabIndex = 0;
        const cellStyle = `padding: 9px 14px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;`;
