...
class EditCollectionInfoPrompt extends InputPrompt {
    constructor(initialName = '', initialDescription = '', initialSize = '', initialStatusCount = 0, collectionUUID, onConfirm, onCancel, onRemove) {
        const confirm = () => {
...
class EditStatusInfoPrompt extends InputPrompt {
    constructor(initialName = '', initialDescription = '', statusUUID, onConfirm, onCancel, onDelete) {
        const confirm = () => {
            const inputs = this.collectInput();
            onConfirm(inputs[0], inputs[1]);
        };
...
export class CollectionEditorUI {
    static get INSTANCE() { return CollectionEditorUI.instance; }
    constructor() {
        // Drag state
        ...
        this.collectionsContainer = document.querySelector('.collections-container');
     ...
     */
    initializeStatusCollection(collectionUUID, name, description) {
        ...
        addStatusButton.addEventListener('click', () => {
            new AddStatusPrompt((name, description) => { CollectionEditor.INSTANCE.addNewStatus(collectionUUID, name, description); }, () => { });
        });
        ...
        cogButton.addEventListener('click', () => {
            new EditCollectionInfoPrompt(collectionName.childNodes[0].textContent || '', descriptionElement.textContent || '', '125 KB', 5, collectionUUID, (newName, newDescription) => {
     ...
     */
    initializeStatusElement(collectionStatusContainer, collectionUUID, statusUUID, name, description = '') {
        ...
        addFlagButton.addEventListener('click', () => {
            ...
            new Promise((resolve) => {
                var _a, _b;
                FlagEditorUI.INSTANCE.createNewFlag((_b = (_a = status.parentElement) === null || _a === void 0 ? void 0 : _a.parentElement) === null || _b === void 0 ? void 0 : _b.getAttribute('data-uuid'), statusUUID, false);
            });
        });
        ...
        // Setup cog button event
        cogButton.addEventListener('click', () => {
            new EditStatusInfoPrompt(statusName.childNodes[0].textContent || '', statusDescription.textContent || '', statusUUID, (newName, newDescription) => {
                // Update the text content without removing child elements
     ...
     */
    setFlagImage(container, imageUUID) {
        container.innerHTML = '';
     ...
     */
    initializeFlagElement(statusFlagContainer, collectionUUID, statusUUID, flagUUID, name, isDefault, imagePath) {
     ...
     */
    getElementInContainerByUUID(UUID, container, elementsSelector) {
     ...
     */
    initializeLoadCollectionButton() {
        const loadBtn = document.querySelector('#main-load-btn');
        loadBtn.addEventListener('click', () => {
            new StatusCollectionFileListViewerPrompt((meta) => {
                CollectionEditor.INSTANCE.loadCollectionByUUID(meta.UUID, true).catch(err => console.error('Failed to load collection:', err));
            }, () => { });
        });
    }
    /**
     ...
     */
    initializeAddCollectionButton() {
        const addCollectionButton = document.querySelector('#main-add-collection-btn');
        ...
        addCollectionButton.addEventListener('click', () => {
            new AddCollectionPrompt((name, description) => { CollectionEditor.INSTANCE.addNewCollection(name, description); }, () => { });
        });
    }
    /**
     * Initialize revert and save buttons.
     */
    initializeRevertAndSaveButtons() {
        const revertButton = document.querySelector('#main-revert-btn');
        const saveButton = document.querySelector('#main-save-btn');
     ...
     */
    updateSaveRevertButtonStates(changesMade) {
     ...
     * Create flag display.
     */
    createFlagDisplay(collectionUUID, statusUUID, flagUUID, isDefault = false) {
        var _a;
        // Get status element
        const statusElement = this.getElementInContainerByUUID(statusUUID, this.collectionsContainer, '.status');
        if (!statusElement)
            throw new Error(`Status element not found for UUID: ${statusUUID}`);
        // Get flags container
        const flagsContainer = statusElement.querySelector('.flags-container');
        if (!flagsContainer)
            throw new Error(`Flags container not found in status UUID: ${statusUUID}`);
        // Get flag data
        const flagData = CollectionEditor.INSTANCE.getFlagByUUID(flagUUID);
        if (!flagData)
            throw new Error(`Flag data not found for UUID: ${flagUUID}`);
        // Initialize flag element
        this.initializeFlagElement(flagsContainer, collectionUUID, statusUUID, flagUUID, flagData.name, isDefault, (_a = flagData.imageUUID) !== null && _a !== void 0 ? _a : null);
    }
    /**
     * Update flag information display.
     */
    updateFlagDisplay(flagUUID) {
        var _a;
        // Find flag element
        const flagElement = this.getElementInContainerByUUID(flagUUID, this.collectionsContainer, '.flag');
        if (!flagElement)
            throw new Error(`Flag element not found for UUID: ${flagUUID}`);
        // Get flag data
        const flagData = CollectionEditor.INSTANCE.getFlagByUUID(flagUUID);
        if (!flagData)
            throw new Error(`Flag data not found for UUID: ${flagUUID}`);
        // Update name
        const nameElement = flagElement.querySelector('.flag-name');
        nameElement.textContent = flagData.name + (flagElement.getAttribute('data-default-flag') === 'true' ? ' (Default)' : '');
        // Update flag image
        const flagImageEl = flagElement.querySelector('.flag-image');
        if (flagImageEl)
            this.setFlagImage(flagImageEl, (_a = flagData.imageUUID) !== null && _a !== void 0 ? _a : null);
    }
    /**
     * Remove a flag from the DOM.
     */
    removeFlagFromDOM(flagUUID) {
        // Find flag element
        const flagElement = this.getElementInContainerByUUID(flagUUID, this.collectionsContainer, '.flag');
        if (!flagElement) {
     ...
     */
    removeStatusFromDOM(statusUUID) {
        // Find status element
        const statusElement = this.getElementInContainerByUUID(statusUUID, this.collectionsContainer, '.status');
        if (!statusElement) {
     ...
     */
    addNewCollectionToDOM(uuid, name, description) {
        this.initializeStatusCollection(uuid, name, description);
    }
    /**
     * Add a new status into an existing collection in the DOM.
     */
    addStatusToCollection(collectionUUID, statusUUID, name, description, defaultFlagUUID) {
        var _a;
        const collectionElement = this.getElementInContainerByUUID(collectionUUID, this.collectionsContainer, '.status-collection');
        if (!collectionElement)
            throw new Error(`Collection not found: ${collectionUUID}`);
        const statusesContainer = collectionElement.querySelector('.statuses-container');
        const statusElement = this.initializeStatusElement(statusesContainer, collectionUUID, statusUUID, name, description);
        const flagData = CollectionEditor.INSTANCE.getFlagByUUID(defaultFlagUUID);
        if (flagData) {
            const flagsContainer = statusElement.querySelector('.flags-container');
            this.initializeFlagElement(flagsContainer, collectionUUID, statusUUID, defaultFlagUUID, flagData.name, true, (_a = flagData.imageUUID) !== null && _a !== void 0 ? _a : null);
        }
    }
    /**
     * Remove a collection from the DOM.
     */
    removeCollectionFromDOM(collectionUUID) {
     ...
     */
    displayFlagCogMenu(flagElement) {
        // Get UUIDs
        const flagUUID = flagElement.getAttribute('data-uuid');
     ...
     */
    enforceAddButtonLast(container, btnClass) {
        const btn = container.querySelector(`:scope > .${btnClass}`);
        if (btn)
    ...
    handleMouseDown(e) {
        ...
        const statusName = target.closest('.status-name');
        if (statusName) {
            const status = statusName.closest('.status');
            if (status) {
     ...
     */
    handleMouseMove(e) {
        if (!this.draggedElement || !this.dragType)
            return;
        const target = e.target;
     ...
     */
    handleFlagDrag(target, e) {
        if (!this.draggedElement)
            return;
        // Find the closest flags container (can be in any status, any collection)
        const flagsContainer = target.closest('.flags-container');
        if (!flagsContainer)
            return;
        ...
        const hoveredFlag = target.closest('.flag');
    ...
    handleMouseUp(e) {
        ...
        this.collectionsContainer.querySelectorAll('.flags-container').forEach(c => this.enforceAddButtonLast(c, 'add-flag-btn'));
        this.collectionsContainer.querySelectorAll('.statuses-container').forEach(c => this.enforceAddButtonLast(c, 'add-status-btn'));
        // Determine if the order has changed and handle accordingly
        if (this.draggedElementPreviousElementParent !== this.draggedElement.parentElement ||
     ...
     */
    translateJSONCollectionIntoDOM(visualStatusCollection) {
        // Create collection
        ...
        // Create statuses
        const statusesContainer = collectionElement.querySelector('.statuses-container');
        visualStatusCollection.statusesUUIDs.forEach((statusUUID) => {
            var _a;
            // Create status
            const statusJSON = CollectionEditor.INSTANCE.getStatusByUUID(statusUUID);
            ...
            // Create default flag
            const defaultFlagJSON = CollectionEditor.INSTANCE.getFlagByUUID(statusJSON.defaultFlagUUID);
            const flagsContainer = statusElement.querySelector('.flags-container');
            this.initializeFlagElement(flagsContainer, visualStatusCollection.UUID, statusUUID, statusJSON.defaultFlagUUID, defaultFlagJSON.name, true, (_a = defaultFlagJSON.imageUUID) !== null && _a !== void 0 ? _a : null);
            // Create flags
            statusJSON.flagUUIDs.forEach((flagUUID) => {
                var _a;
                // Create flag
                const flagJSON = CollectionEditor.INSTANCE.getFlagByUUID(flagUUID);
                this.initializeFlagElement(flagsContainer, visualStatusCollection.UUID, statusUUID, flagUUID, flagJSON.name, false, (_a = flagJSON.imageUUID) !== null && _a !== void 0 ? _a : null);
            });
        });
    }
    // #endregion
    ...
    translateDOMToVisualData() {
        ...
        collectionElements.forEach((collectionElement) => {
            // Get collection info
            const collectionUUID = collectionElement.getAttribute('data-uuid');
            const collectionName = collectionElement.querySelector('.collection-name').textContent || '';
            const collectionDescription = collectionElement.querySelector('.collection-description').textContent || '';
            ...
            statusElements.forEach((statusElement) => {
                // Get status info
                const statusUUID = statusElement.getAttribute('data-uuid');
                const statusName = statusElement.querySelector('.status-name').textContent || '';
                // Prepare status data
                const visualStatus = {
                    UUID: statusUUID,
                    name: statusName,
     ...
     */
    getCollectionAndStatusUUIDFromFlagUUID(flagUUID) {
        // Find flag element
        const flagElement = this.getElementInContainerByUUID(flagUUID, this.collectionsContainer, '.flag');
        if (!flagElement) {
            console.error(`Flag element not found for UUID: ${flagUUID}`);
        ...
        // Get status element
        const statusElement = flagElement.closest('.status');
        if (!statusElement) {
            console.error(`Status element not found for flag UUID: ${flagUUID}`);
            return null;
        }
        // Get collection element
        const collectionElement = statusElement.closest('.status-collection');
        if (!collectionElement) {
        ...
        // Return UUIDs
        return {
            collectionUUID: collectionElement.getAttribute('data-uuid'),
            statusUUID: statusElement.getAttribute('data-uuid')
        };
    }
}
new CollectionEditorUI();
//# sourceMappingURL=collection_editor.js.map