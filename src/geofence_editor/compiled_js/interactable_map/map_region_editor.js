...
import { MapRegionEditorTranslateTool, MapRegionEditorRotateTool, MapRegionEditorScaleTool, MapRegionEditorAddAnchorTool, ToolType } from './map_region_editor_tools.js';
...
import { RegionType, MapRectangleRegion, MapCircleRegion, MapFreeformRegion } from "./region.js";
...
export class MapRegionEditor {
    static get INSTANCE() { return MapRegionEditor.instance; }
    get ActivePrimaryTool() { return this.activePrimaryTool; }
    get IsAddAnchorToolActive() { return this.activeAddAnchorTool !== null; }
    get JustFinishedDragging() { return this.justFinishedDragging; }
    constructor() {
     ...
     */
    setActivePrimaryTool(editorToolType) {
     ...
     */
    setActiveAddAnchorTool(isActive) {
        var _a;
        if (isActive) {
            this.activeAddAnchorTool = new MapRegionEditorAddAnchorTool();
        }
        else {
            (_a = this.activeAddAnchorTool) === null || _a === void 0 ? void 0 : _a.removeTool();
            this.activeAddAnchorTool = null;
        }
...
export class MapRegionAnchorManager {
    static get INSTANCE() { return MapRegionAnchorManager.instance; }
    get CentralizedPoint() { return this.centralizedPoint; }
    get ActiveAnchorPoints() { return this.activeAnchorPoints; }
    get SelectedAnchors() { return this.selectedAnchors; }
    get SelectedHandles() { return this.selectedHandles; }
    get HandleGuideMapPaneID() { return MapRegionAnchorManager.handleGuideMapPaneID; }
    get AnchorMapPaneID() { return MapRegionAnchorManager.anchorMapPaneID; }
    get HandleMapPaneID() { return MapRegionAnchorManager.handleMapPaneID; }
    constructor() {
        // Ensure singleton instance
        ...
        this.anchorStatusMap = new Map();
        this.ghostVisual = null;
        this.ghostVisualActive = false;
        this.ghostGridLines = [];
        this.selectedAnchors = new Set();
    ...
    initializePanes() {
        ...
        this.anchorMapPane.style.zIndex = MAP_LAYER_INDICES['REGION_ANCHORS'];
        // Initialize handle pane
        this.handleMapPane = InteractiveMap.mapInstance.createPane(MapRegionAnchorManager.handleMapPaneID);
        this.handleMapPane.style.zIndex = MAP_LAYER_INDICES['REGION_HANDLES'];
        ...
        this.handleGuideMapPane.style.zIndex = MAP_LAYER_INDICES['REGION_HANDLE_GUIDES'];
    }
    ...
    // #endregion
    // #region Getters
    getCurrentPivotAnchor() { return this.currentPivotAnchor; }
    getCurrentScaleAnchor() { return this.currentScaleAnchor; }
    // #endregion
     ...
     */
    isGhostAnchorActive() { return this.ghostVisualActive; }
    // #endregion
    ...
    createAnchorPoint(latlng, relIncomingHandlePos, relOutgoingHandlePos, updateRegion = true, pushToAnchorPoints = true, insertBetweenClosests = false) {
        ...
        const newAnchor = new AnchorPoint(latlng, relIncomingHandlePos, relOutgoingHandlePos, {
            onAnchorDragStart: (a) => { MapRegionEditor.INSTANCE.onAnchorDragStart(a); },
            onAnchorDrag: (a) => { MapRegionEditor.INSTANCE.onAnchorDrag(a); },
            onAnchorDragEnd: (a) => { MapRegionEditor.INSTANCE.onAnchorDragEnd(a); },
            onHandleDrag: (a, b) => { MapRegionEditor.INSTANCE.onHandleDrag(a, b); },
            onAnchorClick: (a) => { MapRegionEditor.INSTANCE.onAnchorClick(a); },
            onHandleClick: (a, b) => { MapRegionEditor.INSTANCE.onHandleClick(a, b); },
            onAnchorRightClick: (a, e) => { MapRegionEditor.INSTANCE.onAnchorRightClick(a, e); }
        });
    ...
    calculateCentralizedPoint() {
        ...
        if (!this.centralizedPoint) {
            this.centralizedPoint = this.createAnchorPoint(centerLatLng, null, null, false, // Don't update region
     ...
     */
    updateActiveRegionAnchors(newFrontendAnchorData) {
        var _a;
        (_a = MapRegionRegionManager.INSTANCE.ActiveEditingRegion) === null || _a === void 0 ? void 0 : _a.setFrontendAnchorPositions(newFrontendAnchorData);
    }
}
MapRegionAnchorManager.handleGuideMapPaneID = 'region-handle-guide-pane';
MapRegionAnchorManager.anchorMapPaneID = 'region-anchor-pane';
MapRegionAnchorManager.handleMapPaneID = 'region-handle-pane';
/**
 * Class to manage regions within the map region editor.
 */
export class MapRegionRegionManager {
    static get INSTANCE() { return MapRegionRegionManager.instance; }
    get ActiveEditingRegion() { return this.activeEditingRegion; }
    constructor() {
     ...
     */
    loadRegion(regionInput, pushToRegionsArray = true) {
        // Get region data
        ...
        switch (regionData.RegionType) {
            ...
            case RegionType.Rectangle:
                newRegion = new MapRectangleRegion(isUUIDInput ? regionInput : regionData);
                break;
            // Circle
            case RegionType.Circle:
                newRegion = new MapCircleRegion(isUUIDInput ? regionInput : regionData);
                break;
            // Custom Freeform
            case RegionType.Freeform:
                newRegion = new MapFreeformRegion(isUUIDInput ? regionInput : regionData); // Use base rectangle class for custom shapes
                break;
        }
     ...
     */
    loadGeoeditFileContents(geoeditFileData) {
    ...
    updateActiveRegionFrontend() {
        ...
        if (MapRegionAnchorManager.INSTANCE.ActiveAnchorPoints && MapRegionAnchorManager.INSTANCE.ActiveAnchorPoints.length > 0) {
            this.activeEditingRegion.setFrontendAnchorPositions(MapRegionAnchorManager.INSTANCE.ActiveAnchorPoints.map(anchor => ({
    ...
    updateAllRegions() {
        ...
        const regionsWithIndex = [];
        const regionsWithoutIndex = [];
        ...
        const sortedRegions = regionsWithIndex.map(item => item.region).concat(regionsWithoutIndex);
     ...
     */
    attemptStartEditingRegion(UUID) {
        if (this.activeEditingRegion) {
     ...
     */
    loadRegionAnchorsIntoEditor(region) {
        if (!region) {
        ...
        if (region.FrontendShapePointData && region.FrontendShapePointData.length > 0) {
            region.FrontendShapePointData.forEach(pointData => {
                MapRegionAnchorManager.INSTANCE.createAnchorPoint(pointData.anchorPos, pointData.relIncomingHandlePos, pointData.relOutgoingHandlePos, false, // Don't update region yet
    ...
    createRegionFromEditorTriggered(regionType) {
    ...
    startCreatingRegionFromEditor(regionType) {
        if (this.IsCreatingRegionFromEditor) {
     ...
     */
    finishedCreatingRegionFromEditor(mapRegionCreatorEditorHandler) {
    ...
    get IsCreatingRegionFromEditor() { return this.editorRegionCreator !== null; }
    get GetCreatingRegionType() { return this.editorRegionCreator ? this.editorRegionCreator['regionType'] : null; }
    get IsEditingRegion() { return this.activeEditingRegion !== null; }
}
/**
 * Class for managing conversions from region to region data and etc.
 */
export class MapRegionDataManager {
    static get INSTANCE() { return MapRegionDataManager.instance; }
    constructor() {
        // Ensure singleton instance
     ...
     */
    setRegionDataWithUUID(UUID, newRegionData, updateVisualRegion = true, updateEditorPanel = true) {
        var _a;
        ...
        if (updateVisualRegion) {
            ...
            if (updateEditorPanel && ((_a = MapRegionRegionManager.INSTANCE.ActiveEditingRegion) === null || _a === void 0 ? void 0 : _a.RegionData.UUID) === UUID) {
...
export class MapRegionCreatorEditorHandler {
    get RegionType() { return this.regionType; }
     ...
     */
    async getRegionStylePreferences() {
        this.regionStyle = {
    ...
    initializeMouseListeners() {
        ...
        InteractiveMap.mapInstance.on('click', (e) => {
            if (this.creationComplete)
                return; // Ignore when creation is complete
     ...
     */
    createGhostAnchor(latlng) {
        // Create ghost anchor marker
        const ghostAnchorVisual = L.marker(latlng, {
            icon: Visuals.ANCHOR_ICON,
            interactive: false, // Make it non-interactive
            keyboard: false,
            riseOnHover: false,
     ...
     */
    updateRegionShapePoints(mouseLatlng) {
        ...
        switch (this.regionType) {
            case RegionType.Rectangle:
                ...
                const firstPoint = this.placedDataPoints[0];
                const horizontalPoint = mouseLatlng;
            ...
            case RegionType.Circle:
                ...
                const centerPoint = this.placedDataPoints[0];
                const radiusPoint = mouseLatlng;
                this.temporaryRegionDataPoints = [
                    {
    ...
    async finalizeRegionCreation() {
        ...
        MapRegionRegionManager.INSTANCE.setActiveEditingRegion(regionData.UUID);
        // Update the editor UI
...
export class MapRegionEditorKeyStates {
    static get INSTANCE() { return MapRegionEditorKeyStates.instance; }
    get isCtrlPressedDown() { return this.ctrlPressed; }
    get isEscapePressedDown() { return this.escapePressed; }
    get isShiftPressedDown() { return this.shiftPressed; }
    get isAltPressedDown() { return this.altPressed; }
    get isDeletePressedDown() { return this.deletePressed; }
    get isZPressedDown() { return this.zPressed; }
    constructor() {
        this.ctrlPressedDownListeners = [];
        this.escapePressedDownListeners = [];
        this.shiftPressedDownListeners = [];
        this.altPressedDownListeners = [];
        this.deletePressedDownListeners = [];
        this.zPressedDownListeners = [];
        // Ensure singleton instance
    ...
    initKeyboardListeners() {
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Control') {
                ...
                for (const listener of this.ctrlPressedDownListeners) {
                    listener();
                }
                this.ctrlPressed = true;
            }
            else if (e.key === 'Escape') {
                for (const listener of this.escapePressedDownListeners) {
                    listener();
                }
                this.escapePressed = true;
            }
            else if (e.key === 'Shift') {
                for (const listener of this.shiftPressedDownListeners) {
                    listener();
                }
                this.shiftPressed = true;
            }
            else if (e.key === 'Alt') {
                for (const listener of this.altPressedDownListeners) {
                    listener();
                }
                this.altPressed = true;
            }
            else if (e.key === 'Delete') {
                for (const listener of this.deletePressedDownListeners) {
                    listener();
                }
                this.deletePressed = true;
            }
            else if (e.key === 'z') {
                for (const listener of this.zPressedDownListeners) {
                    listener();
                }
                this.zPressed = true;
            }
...
new MapRegionEditor();
//# sourceMappingURL=map_region_editor.js.map